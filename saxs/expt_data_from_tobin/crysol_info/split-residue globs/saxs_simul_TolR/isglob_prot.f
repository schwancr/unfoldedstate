...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      parameter(ns = 50) ! set to the number of experimentaldata points
...
24     format(a3,1x,i2,1x,30(1x,a4))
      enddo
      close(unit=10,status='keep')
...
c      this is the output of the fsglob program !
      open(unit=10,file='fsglob_tolR_rescale13.dat',status='old')
      rewind 10
...
c      initalize scattering intensity profiles and R-factors...
     ...
     &             npdb,totres,respdb,idorig,natpdb,atpdb,rpdb)
       ...
       do i=1,totres
        do j=1,natpdb(i)
         if(globflag(i,j).ne.1)then
          write(6,*)idorig(i),' ',respdb(i),' ',atpdb(i,j);pause
         endif
        enddo
       enddo
...
c      intermediate output - q, I_all_atom(q), I_glob(q)
       open(unit=11,file='is_at_glob_tolR.out',status='unknown')
       rewind 11
       do i=1,ns
       ...
       open(unit=11,file='is_at_tolR.dat',status='unknown')
       rewind 11
       do i=1,ns
        write(11,'(3(e13.6,1x))')si(i),isat(i)
       enddo
       close(unit=11,status='keep')

       open(unit=11,file='is_glob_tolR.dat',status='unknown')
       rewind 11
       do i=1,ns
        write(11,'(3(e13.6,1x))')si(i),isglob(i)
       enddo
       close(unit=11,status='keep')
...
c     the format is q, average correction, rms correction over the pdb files
      open(unit=11,file='is_corr_tolR_rescale13.dat',status='unknown')
      rewind 11
      do i=1,ns
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     & natdat,npdb,totres,respdb,idorig,natpdb,atpdb,rpdb)
...
cccccccccccccccccccccccccccccccc
      ...
      totres = 1
      respdb(totres) = res(1)
      idorig(totres) = nres(1)
      natpdb(totres) = 1
      atpdb(totres,natpdb(totres)) = atom(1)
      do j=1,3
       rpdb(totres,natpdb(totres),j) = r(1,j)
      enddo

      do i=2,npdb
       if(nres(i).ne.nres(i-1))then
        totres = totres+1
        respdb(totres) = res(i)
        idorig(totres) = nres(i)
       endif
       natpdb(totres) = natpdb(totres)+1
       atpdb(totres,natpdb(totres)) = atom(i)
       do j=1,3
        rpdb(totres,natpdb(totres),j) = r(i,j)
       enddo
      enddo
