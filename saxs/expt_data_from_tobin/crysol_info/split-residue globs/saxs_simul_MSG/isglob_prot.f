...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      parameter(ns = 92) ! set to the number of experimentaldata points
...
c      intermediate output - q, I_all_atom(q), I_glob(q)
       open(unit=11,file='is_at_glob.out',status='unknown')
       rewind 11
       do i=1,ns
...
c     the format is q, average correction, rms correction over the pdb files
      open(unit=11,file='is_corr.out',status='unknown')
      rewind 11
      do i=1,ns
