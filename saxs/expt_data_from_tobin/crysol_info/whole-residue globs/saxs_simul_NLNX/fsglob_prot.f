...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      parameter(npdbmax  =  30000)
      parameter(nresmax  =   2000)
      parameter(nglobmax =  10000)

      parameter(ns = 27) ! set this to the numbers of experimental data points
      ...
      double precision rglob(nglobmax,50,3)
      double precision dave(50,50,50),drms(50,50,50),ndave(50,50,50)
      ...
      integer nrescurr,ndres(5),icurr,solflag,bflag
      integer natdat(20),totres,natpdb(nresmax),resbreak(nresmax)
      integer nglob,natglob(100),totglob,idat(nglobmax,50),id0(50)
      integer idres(nglobmax,50),idglob(nglobmax),naver(50)
      ...
      character rescurr*3,temp*4,chain*1
      character resdat(20)*3,atdat(20,40)*4,respdb(nresmax)*3
      character atpdb(nresmax,40)*4,atglob(100,50)*4,resglob(100)*3
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      pi = 3.1415927d0
      ds = 0.01d0
...
24     format(a3,1x,i2,1x,100(1x,a4))
      enddo
      close(unit=10,status='keep')
...
c      split the structure into single-residue globs...
       totglob = 0
       do i=1,totres
        do idg=1,nglob
...
c      calculate globbic form factors...
     ...
     &                 (rpdb(idr1,ida1,3)-rpdb(idr2,ida2,3))**2)
      ...
      write(6,'(f5.3,a,f5.3,a)')
     &store1,' +/- ',store2,' globs per residue'
      do i=1,nglob
       do j=1,ns+1
...
c     use these for cns/xplor and as input for isglob program
      open(unit=10,file='fsglob.out',status='unknown')
      rewind 10
...
c     rms globbic form factors - for your info only 
      open(unit=10,file='fs2glob.out',status='unknown')
      rewind 10
      write(10,1000)(resglob(i),i=1,nglob)
      do i=1,ns+1
       write(10,1010)si(i),(fs2glob(j,i),j=1,nglob)
      enddo
      write(10,1020)' # of instances     ',(naver(i),i=1,nglob)
1020  format(a,40(i5,5x))
      close(unit=10,status='keep')

      open(unit=10,file='atscat.out',status='unknown')
      rewind 10
      write(10,*)'           q           C         O ',
     &           '        N         H         S'
      do i=1,ns+1
       write(10,1010)si(i),(fsat(j,i),j=1,5)
      enddo
      close(unit=10,status='keep')

c     average and rms inter-atomic distances within globs
c     for your info only
      open(unit=10,file='glob_dist.out',status='unknown')
      rewind 10
      do i=1,nglob
       write(10,*)resglob(i),'-----------------------------------'
       write(10,*)'average distances'
       do j=1,natglob(i)
        write(10,1030)(dave(i,j,k),k=1,natglob(i))
1030    format(40(f5.2,2x))
       enddo
       write(10,*)'rms distances'
       do j=1,natglob(i)
        write(10,1030)(drms(i,j,k),k=1,natglob(i))
       enddo
      enddo
      close(unit=10,status='keep')
       ...
       do i=1,npdb
        ...
        if(natpdb(nres(i)).gt.natdat(resid(i)))then
         if((nres(i).ne.1).and.(nres(i).ne.nres(npdb)))then
          write(6,*)'maximum number of atoms exceeded',i
